...
- PUBLISH: YES cho phép công khai repo/mã nguồn và triển khai như mô tả bên dưới; NO chỉ chuẩn bị phát hành cục bộ.
  ...
  Dùng GITHUB_OWNER và REPOSITORY cho repo mã nguồn.
  Dùng tên miền thực tế và đường dẫn triển khai của dự án Vercel đã chọn cho URL game và base path.
...
- Điền rõ VERCEL_SCOPE trước khi liên kết dự án.
  Kiểm tra liên kết Vercel hiện có, báo xung đột đích đến thay vì ghi đè.
...
- Với YES, dùng Vercel CLI đã đăng nhập hoặc nhập Git trong VERCEL_SCOPE.
  Hướng dẫn đăng nhập trình duyệt hay quyền repo khi cần; không hỏi bí mật trong chat.
- Xác minh revision đã triển khai và gameplay công khai không cần đăng nhập.
  Báo URL production thực tế được cấp và kiểm tra chưa chạy.
