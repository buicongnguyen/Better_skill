...
Use the reading toolbar to choose **English**, **Tiếng Việt**, or **한국어**, and toggle **Dark mode**. Each edition includes a quick summary, all 17 full chapters, 64 reading notes, evidence panels, 22 prompt downloads, and ten translated diagrams. Source titles and passage-search terms retain their original English; the separate audit-record download is labeled as English.
...
Twenty-two copyable and downloadable templates cover repository instructions, an optional game review skill, a prompt-generating prompt, a standalone PLAN.md generator, compact and bootstrap Astra prompts, targeted repair, release, a video workflow brief, an environment preflight, and eleven cookbook steps with two publishing alternatives.

The preflight and cookbook recipes begin with editable parameters, followed by a goal, task bullets and a completion check. Copy a recipe into an unsent chat draft and edit its opening values. Agent/model, engine, asset tool, video duration and release fields are included only where useful; choose one option and keep it consistent with the real project. GitHub Pages URLs follow the selected account and repository; Vercel uses the actual assigned project domain. `PUBLISH: NO` prepares locally; `YES` authorizes the named public release. The structure adapts the user-supplied RTS planning example; its readability is not a measured claim of better model performance.
